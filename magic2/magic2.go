...
type Magic2 struct {
	...
	chunkstart[256] uint64 // start of dictionnary chunk - not necessarily a power of two.
...
func (m Magic2) Get(sqt uint8, key uint64) uint8 {
	// inverse key to allow for 0-key !
	key = ^key
	// search for matching key - infinite loop while not found ...
	for i := hash(sqt, key) & (nbkeys - 1); ; i = (ti + 1) & (nbkeys - 1) {		
		if (m.key[i] == key) && (m.sqt[i] == sqt) { // found matching key and sqt !
			// return value
			return m.values[uint64(m.vindex[i]) + m.chunkstart[sqt]]
		}
	}
}
...
type MultiKey struct {
	Key uint64
	Sq4 uint8 // reduced square (0-16) in the first quadrant
}

// Create a new magic2 table in the heap.
// tsize defines the minimum required size per table. Actual table size will be more if needed.
// Each map corresponds to a given table.
// Caution : no key should ever be 64 ones (^uint64(0)) !
func CreateMagic2(tsize []uint64, tables ...map[MultiKey]uint64) (m Magic2) {
	// check inputs
	if len(tables) == 0 {
		return
	}
	if len(tables) > nbtables {
		panic("too many tables")
	}
	if len(tsize) != len(tables) {
		panic("tsize and tables must have same length")
	}

	// create/init internal temporary data structures
	dictionnary := make(map[uint64]uint8, nbvalues) // value -> index

	// handle maps one after the other
	for i, table := range tables {

		// update offset
		m.offsets[i] = 0
		if i > 0 {
			m.offsets[i] = m.offsets[i-1] + m.lengths[i-1]
		}

		// update lengths
		m.lengths[i] = NextPowerOfTwo(max(tsize[i], uint64(len(table))))

		// add all values into dictionnary
		for _, v := range table {
				if _, ok := dictionnary[v];!ok {
				dictionnary[v] = uint8(len(dictionnary))
			}
