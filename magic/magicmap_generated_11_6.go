...
// Generated on 2025-02-27 21:30:42.0657166 +0100 CET m=+0.029853301
// Input : 11 bits (up to 2048 keys) -> Output : 6 bits (up to 64 values)
...
func NewMagicMap_11_6() MagicMap {
	mm := new(magicMap_11_6)
	mm.magic = defaultmagic
	return mm
}

type magicMap_11_6 struct { // capa is 4096 in/64 out
	zerovalue uint64           // value for the '0' key
	keys   [1<<11]uint64 // keyindex ->  input keys
	index  [1<<11]uint8  // keyindex -> valueindex ( NOTE : assumes index < 256 for up to 256 distict output values per magicmap).
...
func (mm *magicMap_11_6) SetMagic(magic uint64) (MagicMap){
	if magic == 0 {
...
func (mm magicMap_11_6) Get(key uint64) (value uint64) {
...
func (mm magicMap_11_6) Size() (bytes int) {
	return int(unsafe.Sizeof((mm)))
}

func (mm magicMap_11_6) Count() (in, out int) {
	return int(mm.nbin), int(mm.nbout)
}

func (mm magicMap_11_6) Capa() (in, out int) {
	return 1<<11, 1 << 6
}

func (mm *magicMap_11_6) Set(key, value uint64) {
	const maxin = 1 << 11
	const maxout = 1 << 6
...
func (mm magicMap_11_6) AllKeys() []uint64 {
...
func (mm magicMap_11_6) AllValues() (values []uint64) {
	return append(values, mm.values[:mm.nbout]...)
}


func (mm magicMap_11_6) Dump() {
...
func (mm magicMap_11_6)  hash(key uint64) uint16 {
	return uint16((key * mm.magic) >> 24) & (1<<11-1) // middle bits have the most entropy
}

// Analyse the collisions in this map
func (mm magicMap_11_6) Stats() (st MagicStats) {
